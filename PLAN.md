...
5. **Movement and ordering safety**
   ...
   - Apply optimistic UI updates and serialize mutations per item. Additionally serialize all position-changing operations, including compound cross-lane moves, per project. Resolve queued movement intent against current state rather than reusing stale anchors or rollback snapshots.
   - Roll back definitive failures or refetch canonical state. Timeouts/disconnections after submission have ambiguous outcomes: reconcile before accepting dependent writes rather than blindly retrying or assuming rollback succeeded.
