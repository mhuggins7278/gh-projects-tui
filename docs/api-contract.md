...
| Writes | `viewerCanUpdate` plus a complete, unfiltered, single-axis single-select/iteration board enables lane moves; position-sorted views also allow reorder. Sorted-by-field, saved-filtered, and combined-axis views are read-only. | Set/clear and top/after-anchor behavior were confirmed on disposable sandbox items. Partial/ambiguous writes, concurrent external anchors, and filtered/combined-axis mutations need further verification and are tracked separately. |
