...
| Writes | `viewerCanUpdate` plus a complete, unfiltered, single-axis single-select/iteration board enables lane moves; position-sorted views also allow reorder. Sorted-by-field, saved-filtered, and combined-axis views are read-only. | Set/clear and top/after-anchor behavior were confirmed on disposable sandbox items. Queue serialization, canonical intent resolution, partial failures, ambiguous outcomes, readback failure, and view switching have fixture coverage. Concurrent external edits during submission and filtered/combined-axis mutations remain outside supported writes. |
...
- Before the first write, and after every submitted mutation, the client paginates the complete unfiltered `items(query:)` result in project-position order. Lane moves and reorders are queued as item/direction intents and resolved against the latest canonical state; queued commands do not retain mutation closures or position anchors.
...
- GraphQL errors and HTTP 4xx responses are treated as definitive rejections; HTTP 5xx, transport/timeouts, malformed/missing mutation payloads, and unknown errors are ambiguous. The client reads back before proceeding. An ambiguous write is accepted when the intended state appears, or resolved as failed/partial after two consecutive readbacks confirm the same relevant state. Otherwise dependent writes remain blocked; pressing `r` retries readback only and never blindly resubmits the write.
